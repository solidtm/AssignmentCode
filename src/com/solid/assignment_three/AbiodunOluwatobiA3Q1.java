 ...
 *
 * PURPOSE: Direct race for a marathon result and display top 20 finishers based on finish time.
 */
public class AbiodunOluwatobiA3Q1 {
    public static void main(String[] args) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            // Sort the race results in increasing order of finish time
            raceResultList.sortResults();

            // Print the top 20 finishers
            int top = 20;
            System.out.println("Top " + top + " Finishers:");
            System.out.println("Place\t\tName\t\tFinish Time\t\tPace per Mile");

            int length = raceResultList.getTopFinishers(top).size();
            List<RaceResult> result = raceResultList.getTopFinishers(top);
            for (int index = 0; index < length; index++) {
                String[] res = {Integer.toString(result.get(index).getBibNumber()), result.get(index).getName(),
                        result.get(index).getFinishTime(), result.get(index).getPacePerMile()};
                System.out.println((index + 1) + "\t\t" + res[1] + "\t\t" + res[2] + "\t\t" + res[3]);
            }
    ...
    public static class RaceResult implements Comparable<RaceResult> {
        ...
        public String toString() {
            return String.format("%d,%s,%.1f,%.1f", bibNumber, name, finishTime, getPacePerMile());
        }
    }
    public static class RaceResultList {
